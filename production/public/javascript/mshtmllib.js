...
(function (m) {
    ...
    if (!g || !j)
      if (
        ...
        if (a === d) {
          ...
          if ('execScript' in d)
            d.execScript('Public ' + b, 'VBScript'), d.execScript('var ' + b + ';', 'JavaScript');
          else
            try {
              z(a, b, { value: s });
            } catch (r) {
  ...
  function V(a) {
    ...
    b &&
      ...
      d.scrollTo(K.scrollLeft || 0, a.top + (K.scrollTop || 0) - (K.clientTop || 0)));
  }
        ...
        if (d.top == d.self) {
          ...
          o
            ...
            : f != b &&
              ((f = f.replace(/([^\/])\?/, '$1/?')),
              RegExp('^' + b, 'i').test(f) &&
                e.replace(b + '#' + f.replace(RegExp('^' + b, 'i'), l.type) + e.hash));
        }
      },
