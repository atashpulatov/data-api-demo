...
describe("StepBindOfficeTable", () => {
  ...
  it("bindOfficeTable should work as expected", async () => {
    ...
    jest
      .spyOn(officeApiDataLoader, "loadSingleExcelData")
      .mockReturnValue("tableNameTest");

    jest.spyOn(officeApiHelper, "bindNamedItem").mockImplementation();

    jest
      .spyOn(operationStepDispatcher, "completeBindOfficeTable")
      .mockImplementation();
    ...
    expect(officeApiHelper.bindNamedItem).toBeCalledWith(
      "tableNameTest",
      "bindIdTest"
    );

    expect(operationStepDispatcher.completeBindOfficeTable).toBeCalledTimes(1);
    expect(operationStepDispatcher.completeBindOfficeTable).toBeCalledWith(
      "objectWorkingIdTest"
    );
  });
  ...
  it.each`
    ...
    ${REFRESH_OPERATION}
  `(
    "should skip bindOfficeTable if no new table created",
    async ({ operationType }) => {
      // given
      const objectData = {
        bindId: "bindIdTest",
      ...
      expect(operationStepDispatcher.completeBindOfficeTable).toBeCalledTimes(
        1
      );
      expect(operationStepDispatcher.completeBindOfficeTable).toBeCalledWith(
        "objectWorkingIdTest"
