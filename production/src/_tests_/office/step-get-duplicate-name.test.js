...
describe("StepGetDuplicateName", () => {
  ...
  `(
    "prepareNewNameForDuplicatedObject should return proper prepared name",
    ({ originalName, expectedResult }) => {
      // given
      // when
      const nameCandidate =
        stepGetDuplicateName.prepareNewNameForDuplicatedObject(originalName);
      // then
  ...
  `(
    ...
    ({ nameCandidate, expectedResult }) => {
      ...
      // when
      const adjustedName =
        stepGetDuplicateName.checkAndSolveNameConflicts(nameCandidate);
      // then
  ...
  it("getDuplicateName should call updateObject with object with new name", () => {
    ...
    const updateObjectMock = jest
      ...
      .mockImplementation();
    const completeGetDuplicateNameMock = jest
      .spyOn(operationStepDispatcher, "completeGetDuplicateName")
      .mockImplementation();
