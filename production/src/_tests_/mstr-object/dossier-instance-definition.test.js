...
describe("DossierInstanceDefinition", () => {
  ...
  it("should handle exception thrown by createDossierInstance", async () => {
    ...
    try {
      ...
      expect(error.mstrObjectType).toEqual(
        mstrObjectEnum.mstrObjectType.dossier.name
      );
    ...
    expect(
      dossierInstanceDefinition.getVisualizationErrorType
    ).not.toBeCalled();
  });
  ...
  it("should handle exception thrown by fetchVisualizationDefinition", async () => {
    ...
    expect(mstrObjectRestService.fetchVisualizationDefinition).toBeCalledTimes(
      1
    );
    ...
    expect(dossierInstanceDefinition.getVisualizationErrorType).toBeCalledTimes(
      1
    );
  ...
  `(
    ...
    async ({
      ...
      const mockCreateDossierInstance = jest.spyOn(
