...
class OfficeDisplayService {
  printObject = async (dossierData, objectId, projectId, isReport = true, selectedCell, officeTableId, bindingId, body, isRefresh, isPrompted, isRefreshAll = false) => {
    if (!isRefreshAll) {
      let objectInfo = null;
      if (isPrompted) {
        objectInfo = await mstrObjectRestService.getObjectInfo(objectId, projectId, isReport);
      } else {
        objectInfo = await mstrObjectRestService.getObjectDefinition(objectId, projectId, isReport);
      }
      reduxStore.dispatch({
