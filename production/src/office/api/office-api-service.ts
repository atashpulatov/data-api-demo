...
class OfficeApiService {
  ...
  async getSelectedCell(excelContext: Excel.RequestContext): Promise<string> {
    try {
      ...
      return officeApiHelper.getStartCellOfRange(selectedRangeStart.address);
    } catch (error) {
      /*
  ...
  async convertCellAddressToRangePosition(
    ...
    const currentExcelSheet = officeApiHelper.getCurrentExcelSheet(excelContext);
  ...
  async onBindingObjectClick(ObjectData: any): Promise<void> {
    ...
    if (isCrosstab) {
      ...
      const tableRange = officeApiHelper.getBindingRange(excelContext, bindId);
      tableRange.select();
    }
    await excelContext.sync();
  }
...
export const officeApiService = new OfficeApiService();
