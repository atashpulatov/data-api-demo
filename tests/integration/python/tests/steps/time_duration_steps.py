...
@step('I saved execution duration to "{timer_name}"')
def step_impl(context, timer_name):
    context.pages.time_duration_page().save_duration(timer_name)


@step('I verified that execution duration "{first_timer_name}" is similar to "{second_timer_name}" '
...
@step('I verified that execution duration "{first_timer_name}" is not longer than "{second_timer_name}"')
