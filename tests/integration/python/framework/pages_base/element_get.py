...
class ElementGet(ElementCheck):
    ...
    def get_element_by_id(self, selector, timeout=Const.DEFAULT_TIMEOUT, safe=False):
        return BaseElement(self._get_raw_element(By.ID, selector, timeout, safe), self.driver)
    ...
    def get_element_by_css(self, selector, timeout=Const.DEFAULT_TIMEOUT, safe=False):
        return BaseElement(self._get_raw_element(By.CSS_SELECTOR, selector, timeout, safe), self.driver)
    ...
    def get_element_by_xpath(self, selector, timeout=Const.DEFAULT_TIMEOUT, image_name=None, safe=False):
        if image_name and self.image_recognition_enabled:
            return ImageElement(self.get_element_info_by_xpath(selector, timeout, image_name, safe=safe), self.driver)
        else:
            return BaseElement(self._get_raw_element(By.XPATH, selector, timeout, safe), self.driver)

    def get_element_by_class_name(self, selector, timeout=Const.DEFAULT_TIMEOUT, image_name=None, safe=False):
        if image_name and self.image_recognition_enabled:
            return ImageElement(self.get_element_info_by_class_name(selector, timeout, image_name, safe), self.driver)
        else:
            return BaseElement(self._get_raw_element(By.CLASS_NAME, selector, timeout, safe), self.driver)

    def get_element_by_name(self, selector, timeout=Const.DEFAULT_TIMEOUT, image_name=None, safe=False):
        if image_name and self.image_recognition_enabled:
            return ImageElement(self.get_element_info_by_name(selector, timeout, image_name, safe), self.driver)
        else:
            return BaseElement(self._get_raw_element(By.NAME, selector, timeout, safe), self.driver)

    def get_element_by_accessibility_id(self, selector, timeout=Const.DEFAULT_TIMEOUT, image_name=None, safe=False):
        if image_name and self.image_recognition_enabled:
            return ImageElement(self.get_element_info_by_mobile_accessibility_id(selector, timeout, safe), self.driver)
        else:
            return BaseElement(self._get_raw_element(MobileBy.ACCESSIBILITY_ID, selector, timeout, safe), self.driver)

    def get_element_by_tag_name(self, selector, timeout=Const.DEFAULT_TIMEOUT, image_name=None, safe=False):
        if image_name and self.image_recognition_enabled:
            return ImageElement(self.get_element_info_by_tag_name(selector, timeout, image_name, safe), self.driver)
        else:
            return BaseElement(self._get_raw_element(By.TAG_NAME, selector, timeout, safe), self.driver)
    ...
    def _get_raw_element(self, selector_type, selector, timeout, safe=False):
