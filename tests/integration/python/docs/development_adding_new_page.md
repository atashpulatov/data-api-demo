...
1. Add information about the Page to all Page Sets to make it available:
    ...
    - `framework/pages_factory/abstract_pages.py` (abstract class implemented by all Page Sets, no implementation here),
        e.g.:
        ```python
        @abstractmethod
        def right_panel_tile_page(self):
            pass
        ```
    - `framework/pages_factory/pages_*.py` (it's necessary to add appropriate method definition to all Page Sets)
