...
task :py_e2e_test_win,[:tag_name, :build_no] do | t, args|
  ...
  begin
    shell_command! "python -m behave --tags=@#{PY_WIN_TEST_PARAM[tag_name]} --no-skipped -D config_file=config_ci_#{PY_WIN_TEST_PARAM[tag_name]}.json --logging-level=DEBUG --format allure_behave.formatter:AllureFormatter -o #{allure_folder} tests/", cwd: test_dir
  ensure
...
task :py_e2e_test_mac,[:tag_name, :build_no] do | t, args|
  ...
  begin 
    shell_command! "behave --tags=@#{PY_MAC_TEST_PARAM[tag_name]} --no-skipped -D config_file=config_ci_#{PY_MAC_TEST_PARAM[tag_name]}.json --logging-level=DEBUG --format allure_behave.formatter:AllureFormatter -o #{allure_folder} tests/", cwd: test_dir
  ensure
    shell_command! "allure generate #{allure_folder} --clean ", cwd: "#{test_dir}"
    info "publish e2e test result to Rally"
    shell_command! "npm install", cwd: get_browser_test_dir()
