...
task :py_e2e_test_mac,[:tag_name, :build_no] do | t, args|
  ...
  begin 
    shell_command! "behave -D config_file=config_ci_#{PY_MAC_TEST_PARAM[tag_name]}.json --logging-level=DEBUG --format allure_behave.formatter:AllureFormatter -o #{allure_folder} tests/features/F25931_duplicate_object", cwd: test_dir
  ensure
    shell_command! "allure generate #{allure_folder} --clean ", cwd: "#{test_dir}"
    info "publish e2e test result to Rally"
