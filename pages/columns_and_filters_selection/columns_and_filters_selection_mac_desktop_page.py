...
class ColumnsAndFiltersSelectionMacDesktopPage(BaseMacDesktopPage):
    ITEM_ALL_ATTRIBUTES = "{prefix}/AXGroup[5]/AXGroup[0]".format(prefix=BaseMacDesktopPage.POPUP_WRAPPER_ELEM)
    ITEM_ALL_METRICS = "{prefix}/AXGroup[7]/AXGroup[0]".format(prefix=BaseMacDesktopPage.POPUP_WRAPPER_ELEM)
    ...
    IMPORT_BUTTON = "{prefix}/AXGroup[13]/AXButton[@AXTitle='Import']".format(prefix=BaseMacDesktopPage.POPUP_WRAPPER_ELEM)
    OFFSET_X = 22
    OFFSET_Y = 15
    ...
    def _click_element(self, selector, element_name):
       formatted_selector = selector.format(prefix=BaseMacDesktopPage.POPUP_WRAPPER_ELEM, attribute=element_name)
        
       self.get_element_by_xpath(formatted_selector).click(offset_x=self.OFFSET_X, offset_y=self.OFFSET_Y)
